...
独立直接采集程序可显式加 `--observe-gaps`，在请求时长内继续记录可恢复的序号/PTS异常和MIPI计数；默认仍在这些异常出现时退出。此选项用于评估已知故障发生频率，不能作为放宽验收标准：检测到连续性或接收错误时，结束仍返回非零状态。连续5次取帧失败、归还错误、格式错误及原有温度停止条件仍立即终止。应使用含此选项支持的新构建，旧二进制不支持。

例如，新构建后可运行 `run_device.py --driver-build BUILD --official-mode full180 --seconds 1800 --nv21 --itp-depth 4 --system-media-lib --observe-gaps --remote-root /root/os04a10-tests`。逐帧记录在结束后落盘，期间每约10秒更新 `progress.json`；需结合最终MIPI快照与完整帧序号判断，不能只依据平均fps或稀疏健康采样。
